...
- 受け付け: `CSV`, `TSV`
- 区切り文字: `,` / `\t` を自動判定
- クォート対応: RFC4180準拠の基本的なクォート（`"..."`, `""`エスケープ）をサポート
- 先頭列: ID/名称ラベル（文字列）
- 2列目以降: 数値特徴量
...
- 先頭セルが `id` / `label` / `name` / `名称` の場合はヘッダー行として扱う
...
- クラスタリング後に可視化用PCAを実行
- `2D` / `3D` を切り替え可能
- 元特徴量次元が可視化次元より小さい場合は不足軸を `0` で補完して表示
...
- ユニットテスト
  - CSV/TSV判定
  - クォート付きCSV（区切り文字を含むラベル）のパース
  - 数値列名ヘッダー（例: `id,2024,2025`）の判定
  - 欠損/非数値行の除外と行番号
  - z-score標準化の性質
  - singletonクラスタを含むSilhouette計算の境界
  - k候補未算出時（小規模データ）の戻り値
  - seed再現性
  - `n_init` 複数実行の慣性比較
  - 特徴量次元不足時を含む2D/3D可視化座標形状
